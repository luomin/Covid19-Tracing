...
func createVehicle(stub shim.ChaincodeStubInterface, args []string) (string, error) {
	...
	//id, err = strconv.FormatInt(nanotime,10)
	//id = strconv.FormatInt(nanotime,10)
	//if err != nil {
	//	return "",err
	//}
        //id =  "how it is possible"
	...
	*/
	...
	//var v = Vehicle{ObjectType: OBJECTTYPE_VEHICLE, UserId: userId, Id: id, Brand: brand, CreateTime: createtime}
...
// Find Vehicle by the state key (Id).
/*
func findGeoDatabyUID(stub shim.ChaincodeStubInterface, args []string) (string, error) {
        if len(args) < 1 {
                return "", fmt.Errorf("There is not enough 1 argument in this function findGeoDatabyUID.")
        }
        var id = strings.TrimSpace(args[0])
        if id == "" {
                return "", fmt.Errorf("The id cannot be blank.")
...
func wronglyTxRandValue(stub shim.ChaincodeStubInterface, args []string) (string, error) {
	...
	var gps = brandChars[p : p+3]

	var v Vehicle = Vehicle{
		ObjectType: OBJECTTYPE_GEODATA,
		Id:         id,
		GPS:        gps,
...
func queryVehiclesByBrand(stub shim.ChaincodeStubInterface, args []string) (string, error) {
	...
	return queryGeoData(stub, []string{fmt.Sprintf(`{"selector":{"%s":"%s","brand":"%s"}}`, OBJECTTYPE_JSON, OBJECTTYPE_GEODATA, brand)})
}
